<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="发明主分出案点数" prop="id" align="center" width="100">
        <template slot-scope="{row}">
          <span>{{ row.fmzfPoint }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="发明副分出案点数" prop="id" align="center" width="100">
        <template slot-scope="{row}">
          <span>{{ row.fmffPoint }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="新型主分出案点数" prop="id" align="center" width="100">
        <template slot-scope="{row}">
          <span>{{ row.xxzfPoint }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="新型副分出案点数" prop="id" align="center" width="100">
        <template slot-scope="{row}">
          <span>{{ row.xxffPoint }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="裁决案件数" prop="id" align="center" width="100">
        <template slot-scope="{row}">
          <span>{{ row.cjNum }}</span>
        </template>
      </el-table-column>
      <el-table-column label="裁决案件点数" prop="id" align="center" width="100">
        <template slot-scope="{row}">
          <span>{{ row.cjPoint }}</span>
        </template>
      </el-table-column>
      <el-table-column label="分类件数合计" prop="id" align="center" width="100">
        <template slot-scope="{row}">
          <span>{{ row.totalCount }}</span>
        </template>
      </el-table-column>
      <el-table-column label="分类点数合计" prop="id" align="center" width="100">
        <template slot-scope="{row}">
...
<script>
import { accountWork } from '@/api/case-statistics'
...
export default {
  ...
  methods: {
    ...
    getList() { // 获取table表格数据
      ...
      accountWork(this.search).then(response => {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['分类代码','姓名', '分类部门', '发明主分出案数','发明主分出案点数', '发明副分出案数','发明副分出案点数', '发明未给号出案数', '新型主分出案数','新型主分出案点数', '新型副分出案数', '新型副分出案点数','新型未给号出案数', '裁决案件数', '裁决案件点数', '分类件数合计', '分类点数合计']
        const filterVal = ['fldm', 'workerName', 'dep1', 'fmzfNumCount','fmzfPoint', 'fmffNumCount','fmffPoint', 'fmNoNumCount', 'xxzfNumCount', 'xxzfPoint','xxffNumCount','xxffPoint', 'xxNoNumCount', 'cjNum', 'cjPoint', 'totalCount', 'totalPoint']
